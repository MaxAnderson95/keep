...
| D15 | **Go + goreleaser, GitHub releases only (no brew yet); MIT** | Single static arm64 binary. No Homebrew tap for now, so path stability is self-managed: `keep apply` stamps keep's own resolved path (`os.Executable()`) into every plist; default home `~/.local/bin/keep` per Max's conventions. Cobra (CLI) + bubbletea (TUI). |
